...
class BatchFixedLagSmoother : public vioFixedLagSmoother {
...
public:
  ...
  /** default constructor */
  BatchFixedLagSmoother(double smootherLag = 0.0, bool apply_bound=false, const LevenbergMarquardtParams& parameters = LevenbergMarquardtParams(), bool enforceConsistency = true) :
  vioFixedLagSmoother(smootherLag), parameters_(parameters), enforceConsistency_(enforceConsistency), APPLY_BOUND(apply_bound) { };
  ...
  virtual bool equals(const vioFixedLagSmoother& rhs, double tol = 1e-9) const;
  ...
  /*********************/
  /* new */
  void setSmartFactorIndex(std::vector<int>& smartfactorIndexVector) {
    newSmartfactorIndexVector_ = smartfactorIndexVector;
  }
...
protected:
  ...
  /** new!!!! **/
  std::vector<int> newSmartfactorIndexVector_;
  std::map<Key,bool> smartfactorKeyMap_;
  bool APPLY_BOUND;

  void updateSmartFactors(const KeyVector& marginalizeKeys);
