...
protected:
  ...
  bool _auto_delete;
public:
  TimeBasedRetriever(bool auto_delete);
  void AddEntry(ros::Time time, T entry);
  T GetClosestEntry(ros::Time &time, bool &success, double &retrieved_time);
  void DeletePrevious(ros::Time time);
  std::vector< std::pair<ros::Time,T> > GetInBetween(ros::Time &last_time, ros::Time &current_time, bool &success);
};

template<typename T>
TimeBasedRetriever<T>::TimeBasedRetriever(bool auto_delete): _auto_delete(auto_delete) {}

template<typename T>
void TimeBasedRetriever<T>::AddEntry(ros::Time time, T entry) {
  Entry e(time, entry);
...
template<typename T>
T TimeBasedRetriever<T>::GetClosestEntry(ros::Time &time, bool &success, double &retrieved_time) {
  ...
  for(size_t i=0; i<_data_list.size(); i++) {
    ...
    if(time > _data_list[i].first) {
      ...
      entry = _data_list[i].second;
      delete_pos = i-1;
      success = true;
      retrieved_time = _data_list[i].first.toSec();
      ...
      if((time-_data_list[i-1].first) > (_data_list[i].first - time)) {
        entry = _data_list[i].second;
        time = _data_list[i].first;//set the time we are fetching
        delete_pos = i-1;
        success = true;
        retrieved_time = _data_list[i].first.toSec();
        ...
        entry = _data_list[i-1].second;
        time = _data_list[i-1].first; //set the time we are fetching
        delete_pos = i-2;
        success = true;
        retrieved_time = _data_list[i].first.toSec();
  ...
  if(!success) {
    ...
    entry = _data_list[_data_list.size()-1].second;
    time = _data_list[_data_list.size()-1].first;
    delete_pos = _data_list.size()-2;
    retrieved_time = _data_list[_data_list.size()-1].first.toSec();
    success = true;
  }
...
template<typename T>
void TimeBasedRetriever<T>::DeletePrevious(ros::Time time) {
  if(_auto_delete) {
...
template<typename T>
std::vector< std::pair<ros::Time,T> > TimeBasedRetriever<T>::GetInBetween(ros::Time &last_time, ros::Time &current_time, bool &success) {
  ...
  for(size_t i=0; i<_data_list.size(); i++) {
    ...
    // last_time is before this time
      ...
      if(i==0) {
        ...
        if ((_data_list[0].first.toNSec()-last_time.toNSec())/1.0e9 > 0.01) {
  ...
  if(!lastFound) {
    ...
    if ((last_time.toNSec()-_data_list[_data_list.size()-1].first.toNSec())/1.0e9 > 0.01) {
  ...
  if(!currFound) {
    ...
    if ((current_time.toNSec()-_data_list[_data_list.size()-1].first.toNSec())/1.0e9 > 0.01) {
