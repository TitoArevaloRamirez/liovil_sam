...
public:
    ...
    VINSmart_estimator(ros::NodeHandle& nh) : cameraposevisual(1,0,0,1), gravity(9.81), initd(false),
        ...
        std::string imuTopic, odomImuTopic, frontendTopic, lidarTopic, lcTopic, odomIMU, odomCamera, cameraPoseVisual, cameraPath, landmarkPub; 
        nh.getParam("imu_topic", imuTopic);
        nh.getParam("odomImu_topic", odomImuTopic);
        ...
        odomImu_sub = nh.subscribe(odomImuTopic, 1000, &VINSmart_estimator::odomImu_callback, this);
    ...
    void odomImu_callback(const nav_msgs::Odometry::ConstPtr& msg) {
        ...
        while(ros::ok()) {
            if (newKeyframe && initd) {
                ...
                while(!keyframeBuffer.empty()) {
                    ...
                    addOdomImu(current_frame_time, current_key_frame.Kid);
                    // add stereo factors
    ...
    void addOdomImu(ros::Time current_frame_time, uint64_t curr_id){
        double correctionTime =  current_frame_time.toSec();
        ...
        {
            if (odomImuQueue.front().header.stamp.toSec() < correctionTime - 0.1)
            {
                // message too old
                odomImuQueue.pop_front();
            }
            else if (odomImuQueue.front().header.stamp.toSec() > correctionTime + 0.1)
            {
                // message too new
                break;
            }
            else
            {
                ...
                gtsam::PriorFactor<gtsam::Pose3> odomImuPriorFactor(X(curr_id), odomImuPose, odomImu_noise);
                graph->add(odomImuPriorFactor);

                std::cout << "\033[1;32m----> VIO:  added odometry \033[0m" << std::endl;
                break;
            }
        }
        return;
