...
#include <vio/time_based_retriever_gps.h>
#include <visual_frontend/StereoFeatureMatches.h>
#include <visual_inertial_odometry/landmarks.h>
...
protected:
    ...
    // gps - FY
    double c_g_time_diff_allow; 
    bool KITTI_pub_gps;
    bool use_gps;
    geodetic_converter::GeodeticConverter g_geodetic_converter;
    TimeBasedRetriever< GPS_MSG > gps_msg_buffer;
    gtsam::noiseModel::Diagonal::shared_ptr gps_noise_model;
    bool init_gps_received;
    ros::Time init_gps_time;
    // local to global
    Eigen::Vector3d init_gps_pos;
    Eigen::Quaterniond init_gps_orien;
    Eigen::Matrix4d init_gps_inv_trans;
...
public:
    ...
    VINSmart_estimator(ros::NodeHandle& nh) : cameraposevisual(1,0,0,1), gravity(9.81), initd(false),
                newKeyframe(false), imuMsg(false),  gps_msg_buffer(false), init_gps_received(false), c_g_time_diff_allow(0.001), last_frame_time(0.0), last_imu_time(0.0), firstIMU(true) {
        // subscribers and publishers 
        std::string imuTopic, frontendTopic, lidarTopic, lcTopic, odomIMU, odomCamera, cameraPoseVisual, cameraPath, landmarkPub ,gps_topic; 
        ...
        // biased GPS noise model
        double gps_sigma_x, gps_sigma_y, gps_sigma_z;
        
        nh.getParam("gps_sigma_x", gps_sigma_x);
        nh.getParam("gps_sigma_y", gps_sigma_y);
        nh.getParam("gps_sigma_z", gps_sigma_z);
        nh.getParam("use_gps", use_gps);
        nh.getParam("KITTI_pub_gps", KITTI_pub_gps);
        gps_noise_model = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(3) << 0.1, 0.1, 0.1).finished()); //Vector 3 is eigen?
        ...
        gps_sub = nh.subscribe(gps_topic, 10000, &VINSmart_estimator::gps_callback, this);
        ...
        landmark_pub = nh.advertise<visual_inertial_odometry::landmarks>(landmarkPub, 100);

        //gps_path_pub = nh.advertise<nav_msgs::Path>("/vio/gps_path", 1000); //FY
        gps_odom_pub = nh.advertise<nav_msgs::Odometry>("vio/gps_odom", 100);
    ...
    void sysInit(const keyframe& first_keyframe, const GPS_MSG& position, const GPS_MSG& velocity) {
        // set frame time
        ...
        std::pair< ros::Time, Eigen::Matrix<double,11,1> > initIMUmeasurement = imuMsg.GetClosestEntry(current_frame_time, imuSuccess);
        // cannot find a closest imu measurement
        if (imuSuccess == false)
            return;
        // a closest imu measurement is found
        Eigen::Quaterniond initialQuat(initIMUmeasurement.second(7),initIMUmeasurement.second(8),initIMUmeasurement.second(9),initIMUmeasurement.second(10));
        ...
        {
            gtsam::Pose3 prior_pose(prior_rotation, position);
            last_optimized_state.pose = prior_pose;            
            last_optimized_state.velocity = velocity;
            std::cout << "system initialized with GPS position: \n" << position << "\n";
            std::cout << "System initialized with GPS velocity: \n" << velocity << "\n";
        }
        ...
        // add GPS
        add_gps_factor(first_keyframe, position, true);   
        ...
        if (use_gps)
            gps_msg_buffer.DeletePrevious(current_frame_time);
    ...
    void frame_callback(const visual_frontend::StereoFeatureMatches::ConstPtr& msg) {
        // get time
        //clock_t start(clock());
        ...
        //GPS_MSG gps_constant_velocity; // for initialization
        ...
        if (!initd) {
            ...
            init_gps_pos << 0, 0, 0;
            GPS_MSG gps_constant_velocity(0,0,0);
            ros::Time cam_time = msg->header.stamp;
            ...
            if ( double(num_feat_thru_N_frame) > featRatio * double(totalCnt) ) {  
                ...
                keyframe KFtoADD;
                KFtoADD.timestamp = cam_time;//libermsg->header.stamp;
                ...
                if (use_gps){
                    if (!init_gps_received || firstIMU)
                        return;
                    std::vector< std::pair<ros::Time, GPS_MSG> > gps_measurements = gps_msg_buffer.GetInBetween(init_gps_time, cam_time, initd);
                    std::pair<ros::Time, GPS_MSG> closest_gps = gps_measurements.back();
                    std::pair<ros::Time, GPS_MSG> sec_closest_gps = gps_measurements[gps_measurements.size()-2];
                    // retrieve IMU message
                    std::pair<ros::Time, Eigen::Matrix<double,11,1>> closest_imu = imuMsg.GetClosestEntry(closest_gps.first, initd);
                    std::pair<ros::Time, Eigen::Matrix<double,11,1>> sec_closest_imu = imuMsg.GetClosestEntry(sec_closest_gps.first, initd);
                    //sys_initd is updated with the two lines above
                    if (!initd)
                        return;
                    gps_constant_velocity = (closest_gps.second - sec_closest_gps.second)
                                            /double(closest_gps.first.toNSec()/1.0e9 - sec_closest_gps.first.toNSec()/1.0e9);
                
                    //TODO: include the velocity check?
                    init_gps_pos = closest_gps.second; //why the second?
                    std::cout << "First raw GPS " << init_gps_pos << std::endl; 
                    init_gps_orien = Eigen::Quaterniond(closest_imu.second(7),closest_imu.second(8), closest_imu.second(9), closest_imu.second(10));
                    
                    Eigen::Matrix<double,3,3> T_enu_ned_;                     
                    T_enu_ned_ << 0, 1, 0,
                                1, 0, 0,
                                0, 0, -1;
                    Eigen::MatrixXd T_enu_ned = Eigen::MatrixXd::Identity(4,4);
                    T_enu_ned.block<3,3>(0,0) = T_enu_ned_;

                    init_gps_inv_trans = Eigen::MatrixXd::Identity(4,4);
                    init_gps_inv_trans.block<3,3>(0,0) = init_gps_orien.normalized().toRotationMatrix();
                    init_gps_inv_trans.block<3,1>(0,3) = init_gps_pos; // T_L_G
                    init_gps_inv_trans = init_gps_inv_trans.inverse().eval(); // T_G_L
                    init_gps_inv_trans = T_enu_ned *init_gps_inv_trans;
                    gps_constant_velocity = init_gps_inv_trans.block<3,3>(0,0) * gps_constant_velocity;
                    init_gps_pos  = init_gps_inv_trans.block<3,3>(0,0) * init_gps_pos;
                }
                // initilize the batch smoother
                sysInit(KFtoADD, init_gps_pos, gps_constant_velocity);
            }
        }
        ...
        while(true) {
            if (newKeyframe && initd) {
                ...
                while(!keyframeBuffer.empty()) {
                    ...
                    if (use_gps)
                        add_gps_factor(current_key_frame, GPS_MSG(), false);
                    // calculated time
                    ...
                    publish_gps_odom(current_key_frame); //FY
                }
    ...
    void add_gps_factor(const keyframe& kf, const GPS_MSG& position, bool provided)
    {
    // position is available as an argument
    if (provided) {
        graph->add(gtsam::GPSFactor(X(kf.Kid), position, gps_noise_model));
    }
    // need to retrieve from the buffer
    else {
        bool gps_success;

        ros::Time current_kf_time = kf.timestamp;
        std::pair<ros::Time, GPS_MSG> closest_gps = gps_msg_buffer.GetClosestEntry(current_kf_time, gps_success);
        if (gps_success && (fabs(closest_gps.first.toNSec()/1.0e9-kf.timestamp.toNSec()/1.0e9) < c_g_time_diff_allow) ) {
            //graph->add(gtsam::GPSFactor(X(kf.Kid), init_gps_inv_trans.block<3,3>(0,0) * closest_gps.second, gps_noise_model));
            graph->add(gtsam::GPSFactor(X(kf.Kid), init_gps_inv_trans.block<3,3>(0,0) * closest_gps.second, gps_noise_model));
        }
    }
    return;
    ...
    void updateLandMark(uint64_t curr_id) {
        ...
        if ( !useISAM2 && toMargiKid>=0 ) {
            ...
            visual_inertial_odometry::landmarks landmarks_msg;
    ...
    void publish_gps_odom(const keyframe& kf){

        nav_msgs::Odometry gps_odom;
        ros::Time current_kf_time = kf.timestamp;
        gps_odom.header.stamp = current_kf_time;
        gps_odom.header.frame_id = "camera_init";

        bool gps_success;
        Eigen::Matrix<double,3,1> current_gps_world(0,0,0);
        Eigen::Matrix<double,3,3> rotz_ALOAM, rotx_lidar_tilt;  //rotz_ALOAM
        rotz_ALOAM << 0, 1, 0,
                    -1, 0, 0,
                    0, 0, 1;

        rotx_lidar_tilt << 1, 0, 0,
                    0, 0.970716481578191, -0.240228042477264,
                    0, 0.240228042477264, 0.970716481578191;                    

        std::pair<ros::Time, GPS_MSG> closest_gps = gps_msg_buffer.GetClosestEntry(current_kf_time, gps_success);
        if (gps_success && (fabs(closest_gps.first.toNSec()/1.0e9-kf.timestamp.toNSec()/1.0e9) < c_g_time_diff_allow) )
            current_gps_world = rotx_lidar_tilt*init_gps_inv_trans.block<3,3>(0,0)*rotz_ALOAM * closest_gps.second;
            //current_gps_world = init_gps_inv_trans.block<3,3>(0,0) * closest_gps.second;

        gps_odom.pose.pose.position.x = current_gps_world[0];
        gps_odom.pose.pose.position.y = current_gps_world[1];
        gps_odom.pose.pose.position.z = current_gps_world[2];
